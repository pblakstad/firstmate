...
FM_MAX_AUTO_NUDGES=3               # consecutive opencode auto-nudges with no task progress before the watchdog gives up and escalates for inspection
FM_AUTO_NUDGE_INTERVAL_SECS=60     # minimum seconds between auto-nudges for the same unchanged task-progress signature
FM_AUTO_NUDGE_MESSAGE=             # generic opencode continuation steer sent by the auto-nudge watchdog; empty uses the built-in default
