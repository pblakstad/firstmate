...
state/               volatile runtime signals; gitignored
  ...
  .hash-* .count-* .stale-* .stale-since-* .wedge-escalations-* .auto-nudge* .auto-nudges-* .seen-* .hb-surfaced-* .last-* .heartbeat-streak   watcher internals; never touch
...
The classifier lives in `bin/fm-classify-lib.sh` and is shared: the captain-relevant verb set, status-scan primitives, opencode auto-nudge watchdog, and provably-working predicate (`crew_is_provably_working`, reusing `bin/fm-crew-state.sh`) back both this always-on watcher and the away-mode daemon, so the overlapping policy cannot drift.
The provably-working predicate runs only on no-verb signal and stale paths, never on every wake, so the per-wake triage stays cheap.
